...
function Tab:AddDropdown(Configs)
    ...
    local DMultiSelect = Configs.Multi or Configs.MultiSelect or false
    ...
    local function UpdateText()
        local selectedNames = {}
        for name, isSelected in pairs(SelectedOptions) do
            if isSelected then
        ...
        if #selectedNames > 0 then
            ActiveLabel.Text = table.concat(selectedNames, ", ")
        else
            ActiveLabel.Text = "..."
        end
    end
    ...
    local function AddOption(Name, Value)
        ...
        OptionButton.Activated:Connect(function()
            if DMultiSelect then
                -- Multi-select mode
                SelectedOptions[Name] = not SelectedOptions[Name]
                SelectedValues[Value] = SelectedOptions[Name]
                
                if SelectedOptions[Name] then
                    CreateTween({IsSelected, "BackgroundTransparency", 0, 0.2})
                    CreateTween({IsSelected, "Size", UDim2.fromOffset(4, 12), 0.2})
                    CreateTween({OptionName, "TextTransparency", 0, 0.2})
                else
                    CreateTween({IsSelected, "BackgroundTransparency", 1, 0.2})
                    CreateTween({IsSelected, "Size", UDim2.fromOffset(4, 4), 0.2})
                    CreateTween({OptionName, "TextTransparency", 0.4, 0.2})
                end
            else
                -- Single-select mode
                for optionName, _ in pairs(SelectedOptions) do
                ...
                for _, optionBtn in pairs(ScrollFrame:GetChildren()) do
                    if optionBtn:IsA("TextButton") and optionBtn.Name == "Option" then
                        local selectedIndicator = optionBtn:FindFirstChildOfClass("Frame")
                        local optionText = optionBtn:FindFirstChildOfClass("TextLabel")
                        
                        if selectedIndicator and optionText then
                            if optionBtn.TextLabel.Text == Name then
                                CreateTween({selectedIndicator, "BackgroundTransparency", 0, 0.2})
                                CreateTween({selectedIndicator, "Size", UDim2.fromOffset(4, 12), 0.2})
                                CreateTween({optionText, "TextTransparency", 0, 0.2})
                            else
                                CreateTween({selectedIndicator, "BackgroundTransparency", 1, 0.2})
                                CreateTween({selectedIndicator, "Size", UDim2.fromOffset(4, 4), 0.2})
                                CreateTween({optionText, "TextTransparency", 0.4, 0.2})
                            end
                        end
                    end
                end
            ...
            SetFlag(Flag, DMultiSelect and SelectedValues or currentSelection[1])
            Funcs:FireCallback(Callback, DMultiSelect and currentSelection or currentSelection[1])
        end)
    ...
    local function GetSelected()
        ...
        return DMultiSelect and selected or selected[1]
    end
    ...
    -- Set default values
    if DMultiSelect and type(OpDefault) == "table" then
        for _, defaultOption in ipairs(OpDefault) do
            if table.find(DOptions, defaultOption) then
                SelectedOptions[defaultOption] = true
                SelectedValues[defaultOption] = true
            end
        end
    elseif not DMultiSelect and OpDefault then
        if table.find(DOptions, OpDefault) then
            SelectedOptions[OpDefault] = true
            SelectedValues[OpDefault] = true
        end
    end
    ...
    function Dropdown:Callback(...) 
        local currentSelection = GetSelected()
        Funcs:InsertCallback(Callback, ...)(currentSelection) 
    end

    function Dropdown:Add(...)
        local NewOptions = {...}
        if type(NewOptions[1]) == "table" then
            for _, option in ipairs(NewOptions[1]) do
                AddOption(option, option)
            end
        else
    ...
    function Dropdown:Select(Option)
        if type(Option) == "string" then
            for _, optionBtn in pairs(ScrollFrame:GetChildren()) do
                if optionBtn:IsA("TextButton") and optionBtn.Name == "Option" then
                    if optionBtn.TextLabel.Text == Option then
                        optionBtn.Activated:Fire()
                        break
                    end
                end
            end
    ...
    function Dropdown:Set(Val1, Clear)
        if type(Val1) == "table" then
            ...
            for _, option in ipairs(Val1) do
                AddOption(option, option)
            end
            CalculateSize()
        elseif type(Val1) == "function" then
...
function Tab:AddInput(Configs)
    local IName = Configs[1] or Configs.Name or Configs.Title or "Input"
    local IDesc = Configs.Desc or Configs.Description or ""
    local IDefault = Configs[2] or Configs.Value or Configs.Default or ""
    local IPlaceholder = Configs[5] or Configs.Placeholder or "Enter text..."
    local IInputIcon = Configs.InputIcon or ""
    local IType = Configs.Type or "Input" -- "Input" atau "Textarea"
    local Callback = Funcs:GetCallback(Configs, 4)
    
    -- Konversi icon
    IInputIcon = redzlib:GetIcon(IInputIcon)
    if not IInputIcon:find("rbxassetid://") or IInputIcon:gsub("rbxassetid://", ""):len() < 6 then
        IInputIcon = false
    end

    local Button, LabelFunc = ButtonFrame(Container, IName, IDesc, UDim2.new(1, IInputIcon and -55 or -38))

    local InputFrame = InsertTheme(Create("Frame", Button, {
        Size = IType == "Textarea" and UDim2.new(0, 150, 0, 40) or UDim2.new(0, 150, 0, 18),
        Position = UDim2.new(1, -10, 0.5),
        AnchorPoint = Vector2.new(1, 0.5),
        BackgroundColor3 = Theme["Color Stroke"]
    }), "Stroke")Make("Corner", InputFrame, UDim.new(0, 4))

    local TextBoxInput
    if IType == "Textarea" then
        TextBoxInput = InsertTheme(Create("TextBox", InputFrame, {
            Size = UDim2.new(0.9, 0, 0.85, 0), -- Lebih lebar karena tidak ada icon
            AnchorPoint = Vector2.new(0.5, 0.5),
            Position = UDim2.new(0.5, 0, 0.5, 0),
            BackgroundTransparency = 1,
            Font = Enum.Font.Gotham,
        ...
        TextBoxInput:GetPropertyChangedSignal("Text"):Connect(function()
            local lineCount = select(2, string.gsub(TextBoxInput.Text, "\n", "")) + 1
            local newHeight = math.clamp(lineCount * 12 + 10, 40, 100)
            CreateTween({InputFrame, "Size", UDim2.new(0, 150, 0, newHeight), 0.2})
        end)
    else
        TextBoxInput = InsertTheme(Create("TextBox", InputFrame, {
            Size = IInputIcon and UDim2.new(0.75, 0, 0.85, 0) or UDim2.new(0.9, 0, 0.85, 0),
            AnchorPoint = Vector2.new(0.5, 0.5),
            Position = IInputIcon and UDim2.new(0.6, 0, 0.5, 0) or UDim2.new(0.5, 0, 0.5, 0),
            BackgroundTransparency = 1,
            Font = Enum.Font.Gotham,
            TextColor3 = Theme["Color Text"],
            ClearTextOnFocus = false,
    ...
    -- Icon input (jika ada) - ditempatkan di kiri
    if IInputIcon then
        local InputIcon = Create("ImageLabel", InputFrame, {
            Size = UDim2.new(0, 14, 0, 14),
            Position = UDim2.new(0, 8, 0.5),
            AnchorPoint = Vector2.new(0, 0.5),
            Image = IInputIcon,
    ...
    -- Ketika selesai input
    TextBoxInput.FocusLost:Connect(function(enterPressed)
        IsFocused = false
        HandleInput()
        
        -- Efek visual saat unfocus
        if IType == "Input" then
            CreateTween({InputFrame, "BackgroundColor3", Theme["Color Stroke"], 0.2})
        end
    end)
    ...
    TextBoxInput.Focused:Connect(function()
        ...
        -- Efek visual saat focus
        if IType == "Input" then
            CreateTween({InputFrame, "BackgroundColor3", Theme["Color Theme"], 0.2})
        end
    end)

    -- Enter untuk submit (khusus single line)
    if IType == "Input" then
        TextBoxInput.FocusLost:Connect(function(enterPressed)
    ...
    function Input:SetPlaceholder(NewPlaceholder)
        if type(NewPlaceholder) == "string" then
            TextBoxInput.PlaceholderText = NewPlaceholder
        end
    end
    
    function Input:Set(Val1, Val2)
        if type(Val1) == "string" and type(Val2) == "string" then
            LabelFunc:SetTitle(Val1)
            LabelFunc:SetDesc(Val2)
        elseif type(Val1) == "string" then
        ...
        elseif type(Val1) == "table" then
            if Val1.Value then
                self:SetValue(Val1.Value)
            end
            if Val1.Placeholder then
                self:SetPlaceholder(Val1.Placeholder)
            end
        end
    end
